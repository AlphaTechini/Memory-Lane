// Question bank for replica creation
export const REQUIRED_QUESTIONS = [
  {
    id: 'rq1',
    segment: 'required',
    text: 'What drives you most in life? Describe your core motivations, values, and what gives your life meaning.',
    type: 'textarea',
    required: true,
    minLength: 150
  },
  {
    id: 'rq2',
    segment: 'required',
    text: 'How do you handle failure and setbacks? Walk me through a time when things didn\'t go as planned and how you dealt with it.',
    type: 'textarea',
    required: true,
    minLength: 150
  },
  {
    id: 'rq3',
    segment: 'required',
    text: 'What are your biggest fears, and how do they influence your decisions and behavior?',
    type: 'textarea',
    required: true,
    minLength: 150
  },
  {
    id: 'rq4',
    segment: 'required',
    text: 'Describe your ideal relationship (romantic, friendship, or family). What do you value most in human connections?',
    type: 'textarea',
    required: true,
    minLength: 150
  },
  {
    id: 'rq5',
    segment: 'required',
    text: 'If you could change one thing about the world, what would it be and why? How would you go about making that change?',
    type: 'textarea',
    required: true,
    minLength: 150
  },
  {
    id: 'rq6',
    segment: 'required',
    text: 'What do you believe happens after death? How does this belief influence how you live your life?',
    type: 'textarea',
    required: true,
    minLength: 150
  },
  {
    id: 'rq7',
    segment: 'required',
    text: 'Describe a moment when you felt most proud of yourself. What did you accomplish and why was it meaningful?',
    type: 'textarea',
    required: true,
    minLength: 150
  },
  {
    id: 'rq8',
    segment: 'required',
    text: 'How do you define success? Has this definition changed over time, and if so, how?',
    type: 'textarea',
    required: true,
    minLength: 150
  },
  {
    id: 'rq9',
    segment: 'required',
    text: 'What\'s your biggest regret, and what have you learned from it? How has it shaped who you are today?',
    type: 'textarea',
    required: true,
    minLength: 150
  },
  {
    id: 'rq10',
    segment: 'required',
    text: 'If you had unlimited resources and time, what would you dedicate your life to? What legacy would you want to leave?',
    type: 'textarea',
    required: true,
    minLength: 150
  }
];

export const OPTIONAL_SEGMENTS = {
  occupation: {
    name: 'Occupation & Career',
    description: 'Questions about your work life, career goals, and professional experiences',
    questions: [
      {
        id: 'occ1',
        segment: 'occupation',
        text: 'What does a typical workday look like for you? Walk me through your daily routine.',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'occ2',
        segment: 'occupation',
        text: 'What originally drew you to your current field or profession?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'occ3',
        segment: 'occupation',
        text: 'Describe your biggest professional achievement and what it meant to you.',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'occ4',
        segment: 'occupation',
        text: 'What\'s the most challenging aspect of your work, and how do you handle it?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'occ5',
        segment: 'occupation',
        text: 'Where do you see your career heading in the next 5-10 years?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'occ6',
        segment: 'occupation',
        text: 'What skills are you currently developing or want to learn?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'occ7',
        segment: 'occupation',
        text: 'How do you handle workplace conflicts or difficult colleagues?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'occ8',
        segment: 'occupation',
        text: 'What advice would you give to someone starting in your field?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'occ9',
        segment: 'occupation',
        text: 'How do you balance work and personal life?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'occ10',
        segment: 'occupation',
        text: 'What\'s your ideal work environment like?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'occ11',
        segment: 'occupation',
        text: 'Describe a time when you had to lead a team or project.',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'occ12',
        segment: 'occupation',
        text: 'What motivates you to do your best work?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'occ13',
        segment: 'occupation',
        text: 'How do you stay updated with trends and changes in your industry?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'occ14',
        segment: 'occupation',
        text: 'What\'s the biggest mistake you\'ve made professionally and what did you learn?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'occ15',
        segment: 'occupation',
        text: 'If you could change careers tomorrow, what would you do?',
        type: 'textarea',
        required: false,
        minLength: 40
      }
    ]
  },
  hobbies: {
    name: 'Hobbies & Interests',
    description: 'Questions about your passions, recreational activities, and personal interests',
    questions: [
      {
        id: 'hob1',
        segment: 'hobbies',
        text: 'What hobby or activity makes you lose track of time?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'hob2',
        segment: 'hobbies',
        text: 'How did you discover your main interests or hobbies?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'hob3',
        segment: 'hobbies',
        text: 'What\'s something you\'ve always wanted to learn or try?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'hob4',
        segment: 'hobbies',
        text: 'Describe your favorite way to spend a free weekend.',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'hob5',
        segment: 'hobbies',
        text: 'What books, movies, or shows have significantly impacted you?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'hob6',
        segment: 'hobbies',
        text: 'Do you prefer indoor or outdoor activities? Why?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'hob7',
        segment: 'hobbies',
        text: 'What\'s your favorite genre of music and why does it resonate with you?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'hob8',
        segment: 'hobbies',
        text: 'Have you ever created something you\'re particularly proud of?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'hob9',
        segment: 'hobbies',
        text: 'What sports or physical activities do you enjoy?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'hob10',
        segment: 'hobbies',
        text: 'How do you like to celebrate special occasions or achievements?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'hob11',
        segment: 'hobbies',
        text: 'What\'s your favorite place you\'ve traveled to and why?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'hob12',
        segment: 'hobbies',
        text: 'Do you collect anything? What draws you to it?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'hob13',
        segment: 'hobbies',
        text: 'What\'s your ideal vacation like?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'hob14',
        segment: 'hobbies',
        text: 'How do you discover new interests or hobbies?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'hob15',
        segment: 'hobbies',
        text: 'What role does creativity play in your life?',
        type: 'textarea',
        required: false,
        minLength: 40
      }
    ]
  },
  viewpoints: {
    name: 'Viewpoints & Beliefs',
    description: 'Questions about your opinions, beliefs, and perspectives on various topics',
    questions: [
      {
        id: 'view1',
        segment: 'viewpoints',
        text: 'What\'s your philosophy on money and financial success?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'view2',
        segment: 'viewpoints',
        text: 'How do you feel about social media and its impact on society?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'view3',
        segment: 'viewpoints',
        text: 'What\'s your stance on environmental responsibility?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'view4',
        segment: 'viewpoints',
        text: 'How do you think technology is changing human relationships?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'view5',
        segment: 'viewpoints',
        text: 'What\'s your opinion on the role of government in people\'s lives?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'view6',
        segment: 'viewpoints',
        text: 'How do you approach controversial or sensitive topics in conversation?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'view7',
        segment: 'viewpoints',
        text: 'What\'s your perspective on education and learning?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'view8',
        segment: 'viewpoints',
        text: 'How do you feel about globalization and cultural diversity?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'view9',
        segment: 'viewpoints',
        text: 'What\'s your opinion on the balance between privacy and security?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'view10',
        segment: 'viewpoints',
        text: 'How do you think artificial intelligence will impact the future?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'view11',
        segment: 'viewpoints',
        text: 'What\'s your perspective on traditional values versus progressive change?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'view12',
        segment: 'viewpoints',
        text: 'How do you approach making ethical decisions?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'view13',
        segment: 'viewpoints',
        text: 'What\'s your opinion on the importance of mental health awareness?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'view14',
        segment: 'viewpoints',
        text: 'How do you feel about the role of religion or spirituality in modern life?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'view15',
        segment: 'viewpoints',
        text: 'What\'s your stance on work-life balance and career ambition?',
        type: 'textarea',
        required: false,
        minLength: 40
      }
    ]
  },
  communication: {
    name: 'Communication Style',
    description: 'Questions about how you interact, communicate, and relate to others',
    questions: [
      {
        id: 'comm1',
        segment: 'communication',
        text: 'How would your closest friends describe your personality?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'comm2',
        segment: 'communication',
        text: 'What\'s your communication style when you\'re upset or angry?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'comm3',
        segment: 'communication',
        text: 'How do you prefer to give and receive feedback?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'comm4',
        segment: 'communication',
        text: 'Describe how you make new friends or build relationships.',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'comm5',
        segment: 'communication',
        text: 'What makes you feel most understood by others?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'comm6',
        segment: 'communication',
        text: 'How do you handle disagreements or conflicts?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'comm7',
        segment: 'communication',
        text: 'What\'s your preferred way to show affection or care for others?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'comm8',
        segment: 'communication',
        text: 'How do you like to be comforted when you\'re going through a tough time?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'comm9',
        segment: 'communication',
        text: 'What topics do you find most engaging in conversation?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'comm10',
        segment: 'communication',
        text: 'How do you express excitement or enthusiasm?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'comm11',
        segment: 'communication',
        text: 'What\'s your approach to small talk versus deep conversations?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'comm12',
        segment: 'communication',
        text: 'How do you like to celebrate with others?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'comm13',
        segment: 'communication',
        text: 'What\'s your humor style? What makes you laugh?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'comm14',
        segment: 'communication',
        text: 'How do you prefer to receive support when facing challenges?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'comm15',
        segment: 'communication',
        text: 'What\'s your approach to maintaining long-distance relationships?',
        type: 'textarea',
        required: false,
        minLength: 40
      }
    ]
  },
  lifestyle: {
    name: 'Lifestyle & Habits',
    description: 'Questions about your daily routines, habits, and lifestyle choices',
    questions: [
      {
        id: 'life1',
        segment: 'lifestyle',
        text: 'Describe your ideal morning routine.',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'life2',
        segment: 'lifestyle',
        text: 'How do you maintain your physical and mental health?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'life3',
        segment: 'lifestyle',
        text: 'What does your living space say about you?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'life4',
        segment: 'lifestyle',
        text: 'How do you organize your time and priorities?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'life5',
        segment: 'lifestyle',
        text: 'What\'s your relationship with food and cooking?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'life6',
        segment: 'lifestyle',
        text: 'How do you unwind and relax after a stressful day?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'life7',
        segment: 'lifestyle',
        text: 'What habits are you trying to build or break?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'life8',
        segment: 'lifestyle',
        text: 'How do you approach shopping and spending money?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'life9',
        segment: 'lifestyle',
        text: 'What\'s your sleep schedule and bedtime routine like?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'life10',
        segment: 'lifestyle',
        text: 'How do you stay organized in your personal life?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'life11',
        segment: 'lifestyle',
        text: 'What role does fashion or personal style play in your life?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'life12',
        segment: 'lifestyle',
        text: 'How do you balance spontaneity with planning?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'life13',
        segment: 'lifestyle',
        text: 'What\'s your approach to learning new skills or knowledge?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'life14',
        segment: 'lifestyle',
        text: 'How do you handle household chores and responsibilities?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'life15',
        segment: 'lifestyle',
        text: 'What role does routine versus variety play in your daily life?',
        type: 'textarea',
        required: false,
        minLength: 40
      }
    ]
  },
  quirks: {
    name: 'Quirks & Personality',
    description: 'Questions about your unique traits, habits, and personal quirks',
    questions: [
      {
        id: 'quirk1',
        segment: 'quirks',
        text: 'What\'s a weird or unique habit you have that most people don\'t know about?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'quirk2',
        segment: 'quirks',
        text: 'What\'s something that annoys you that most people probably wouldn\'t mind?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'quirk3',
        segment: 'quirks',
        text: 'Describe a superstition or ritual you follow, even if you know it\'s not logical.',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'quirk4',
        segment: 'quirks',
        text: 'What\'s your most irrational fear?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'quirk5',
        segment: 'quirks',
        text: 'What\'s something you find hilarious that others might not understand?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'quirk6',
        segment: 'quirks',
        text: 'Do you have any collections or obsessions? What draws you to them?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'quirk7',
        segment: 'quirks',
        text: 'What\'s a conspiracy theory or unpopular opinion you secretly believe in?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'quirk8',
        segment: 'quirks',
        text: 'Describe your most embarrassing moment and how you handled it.',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'quirk9',
        segment: 'quirks',
        text: 'What\'s something you do when you think nobody is watching?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'quirk10',
        segment: 'quirks',
        text: 'What\'s your guilty pleasure that you\'re slightly ashamed to admit?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'quirk11',
        segment: 'quirks',
        text: 'How do you act when you\'re home alone versus when others are around?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'quirk12',
        segment: 'quirks',
        text: 'What\'s a childhood trait or behavior you\'ve never grown out of?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'quirk13',
        segment: 'quirks',
        text: 'What\'s your weirdest talent or party trick?',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'quirk14',
        segment: 'quirks',
        text: 'Describe your most vivid recurring dream or nightmare.',
        type: 'textarea',
        required: false,
        minLength: 40
      },
      {
        id: 'quirk15',
        segment: 'quirks',
        text: 'What\'s something about you that would surprise people who know you well?',
        type: 'textarea',
        required: false,
        minLength: 40
      }
    ]
  }
};

// Helper function to get all optional questions
export function getAllOptionalQuestions() {
  return Object.values(OPTIONAL_SEGMENTS).flatMap(segment => segment.questions);
}

// Helper function to get questions by segment
export function getQuestionsBySegment(segmentKey) {
  return OPTIONAL_SEGMENTS[segmentKey]?.questions || [];
}

// Helper function to get questions by segment keys
export function getQuestionsBySegments(segmentKeys) {
  return segmentKeys.flatMap(key => getQuestionsBySegment(key));
}
