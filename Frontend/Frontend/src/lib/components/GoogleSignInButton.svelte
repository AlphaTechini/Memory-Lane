<script>
  ...
  async function handleGoogleSignIn() {
    ...
    // Firebase not initialized → show bubble
    if (!firebaseAuth) {
      console.error('Google sign-in error: Firebase not configured');
      showToastMessage('Feature not available yet');
      return;
    }
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        showToastMessage(data.message || 'Google sign-in failed');
      }
    } catch (err) {
      console.error('Google sign-in error:', err);
      showToastMessage(err.message || 'Failed to sign in with Google');
    } finally {
      loading = false;
    }
  }
...
{#if showToast}
  <div
    class="fixed bottom-6 right-6 bg-gray-900 text-white px-4 py-2 rounded-lg shadow-lg"
    transition:fade
  >
    {toastMessage}
  </div>
