<script>
  // All original script content is kept, but handleGoogleSignIn is now irrelevant
  // as the button is permanently disabled.
  import { goto } from '$app/navigation';
  import { apiUrl } from '$lib/utils/api.js';
  import { initFirebaseClient } from '$lib/firebase';

  // Changed 'disabled' default to true to reflect the new requirement
  let { mode = 'signin', disabled = true } = $props();
  let loading = $state(false);
  let error = $state(null);

  // This function is no longer called, but kept for context/future use
...
<div class="space-y-2">
  <div class="relative group">
    <button
      ...
      disabled={true} class="w-full flex items-center justify-center gap-3 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
    >
      {#if loading}
    ...
    <div class="absolute z-10 top-0 left-1/2 -translate-x-1/2 -translate-y-full px-3 py-1 mt-2 text-sm text-white bg-gray-800 rounded-md whitespace-nowrap opacity-0 transition-opacity duration-300 pointer-events-none group-hover:opacity-100 dark:bg-gray-200 dark:text-gray-900">
        Feature not available yet
        <div class="absolute bottom-[-6px] left-1/2 -translate-x-1/2 w-0 h-0 border-l-4 border-l-transparent border-r-4 border-r-transparent border-t-4 border-t-gray-800 dark:border-t-gray-200"></div>
    </div>
  </div>
