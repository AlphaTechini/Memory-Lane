...
>
  ...
  >
    ...
    <div
      class="message-bubble {isUser ? 'message-bubble-user' : 'message-bubble-bot'}"
    >
      ...
      {#if hasWebSearch}
        <div class="mt-3 pt-2 border-t border-current/20 flex items-center gap-2">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
          </svg>
          <span class="text-accessible-sm font-medium opacity-80">Includes web search results</span>
        </div>
      {/if}
    </div>
