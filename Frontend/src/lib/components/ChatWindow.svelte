...
<div class="flex flex-col h-full bg-cream-50 dark:bg-charcoal-900">
    ...
    {#if messages.length === 0}
      ...
      <div class="flex justify-center items-center h-full">
        <div class="text-center card-accessible max-w-md mx-auto">
          <div class="mb-6">
            <svg class="w-20 h-20 mx-auto text-teal-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>
            </svg>
          </div>
          <h2 class="text-accessible-xl font-bold text-charcoal-800 dark:text-cream-100 mb-3">
    ...
    {:else}
      ...
      {#if suggestedQuestions.length > 0 && messages.length <= 2 && onQuestionSelect}
        <div class="mt-6 card-accessible">
          <h3 class="text-accessible-lg font-semibold text-charcoal-800 dark:text-cream-100 mb-4">
            Suggested questions:
          </h3>
          <div class="space-y-3">
            {#each suggestedQuestions as question (question)}
              <button
                ...
                class="btn-tactile btn-tactile-secondary w-full text-left justify-start"
              >
                <svg class="w-5 h-5 flex-shrink-0 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
