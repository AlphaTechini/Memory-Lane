<script>
  ...
  const { photo, albums = [], showControls = true, showAlbumSelector = false, showDownload = true, isCover = false, canDelete = true } = $props();
...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-md transition-shadow group">
  <div class="aspect-square relative">
    <button
      type="button"
      class="w-full h-full p-0 m-0 border-none bg-transparent object-cover cursor-pointer"
      onclick={handleView}
  ...
  <div class="p-3">
    {#if isEditing}
      ...
      <div class="space-y-3">
        <div>
          <label for="photo-description-input" class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
        ...
        {#if showAlbumSelector}
          <div>
            <label for="photo-album-select" class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Album</label>
            <select
