...
const browserLaunchOptions = {
    // Headless mode is crucial for server environments. Setting to 'true' prevents a visible browser window
    // from opening, which can cause the process to hang, especially on servers without a GUI.
    headless: true,
    args: [
...
const startServer = async () => {
  ...
  await fastify.register(fastifyCors, {     // `origin: true` reflects the request's origin, which is a good default for development.
  ...
  fastify.get('/api/scrape', async (request, reply) => {
    ...
    try {
      ...
      let data;
      // Reduced timeout to 60 seconds per page for faster feedback to the user.
      const TIMEOUT_PER_PAGE = 60000; // 60 seconds
  ...
  fastify.post('/api/search', async (request, reply) => {
    ...
    try {
      ...
      [chromiumBrowser, webkitBrowser] = await Promise.all([
        ...
        playwrightExtra.webkit.launch({ ...browserLaunchOptions, headless: true }) // WebKit doesn't support all args, but headless is key.
      ]);
      ...
      const scrapePromises = urls.map(async (url, index) => {
        ...
        try {
          ...
          const TIMEOUT_PER_URL = 60000; // 60 seconds base timeout
          if (numPages > 1) {
            // The timeout should wrap the entire paginated scrape for one base URL.
            // Adjust timeout based on number of pages.
            return await withTimeout(scrapePaginatedLogic(page, url, numPages), TIMEOUT_PER_URL * numPages);
          } else {
            // Just scraping the single landing page.
            const result = await withTimeout(scrapePageLogic(page, url), TIMEOUT_PER_URL);
